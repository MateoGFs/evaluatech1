...
function Grupos() {
    ...
    const [idGrupo, setIdGrupo] = useState('')//Para modificar obtengo id del grupo
    ...
    const obtenerGrupoEditar = async(idParametro)=>{//Para cargar los datos en el form para editar
        const id = idParametro
        const token = sessionStorage.getItem('token')
        const respuesta= await Axios.get('/grupo/listar/'+id,{
            headers:{'autorizacion':token} 
        })
        console.log(respuesta.data);
        setIdGrupo(respuesta.data._id)
    ...
    const actualizar = async(e)=>{
        ...
        const respuesta= await Axios.put('/grupo/actualizar/'+id,newGrupo,{
            headers:{'autorizacion':token}
        })
    ...
    const eliminar = async(id)=>{
        const token = sessionStorage.getItem('token')
        const respuesta = await Axios.delete('/grupo/eliminar/'+id,{headers:{'autorizacion':token}}) 
        const mensaje=respuesta.data.mensaje
        Swal.fire({ 
            icon: 'error',
            title: mensaje,
    ...
    const crear = async(e)=>{
        ...
        const token = sessionStorage.getItem('token')
        const respuesta=await Axios.post('/grupo/crear',nuevoGrupo,{headers:{'autorizacion':token}})
    ...
    const buscar=async(e)=>{
        ...
        const token= sessionStorage.getItem('token')
        const respuesta= await Axios.get(`/grupo/buscar/${buscar}/${sessionStorage.getItem('idusuario')}`,{
            headers:{'autorizacion':token}
                ...
                <div className="container">
                    <div className="row">
                        <div className="col-md-6">
                            <h1><i className='fas fa-users'> Grupos</i></h1>
                        </div>
                    </div>
                </div>
            </header>      
                                ...
                                <tbody>
                                    {
                                        grupos.map((grupo,i)=>(
                                            ...
                                            <tr key={grupo._id}>
                                                ...
                                                <td>{grupo.nombre}</td>                                             
                                                <td>
                                                    <button className='btn btn-warning mr-1' data-toggle="modal" data-target="#editGrupo" onClick={()=>obtenerGrupoEditar(grupo._id)}>Editar</button>
                                                    <button className='btn btn-danger mr-1' onClick={()=>eliminar(grupo._id)}>Eliminar</button>
                                                </td>
                                            </tr>
                                        ))
                                   }
